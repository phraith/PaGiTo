...
namespace CpuInfo
{
	...
	std::vector<std::string> GetCores(const std::string &cpu_core_dir)
	{
		...
		if (cpu_core_f.good())
		{
			...
			return boost::split(core_split, core_str, boost::is_any_of(","));
		}
	...
	int FindCacheSize(int level, std::string type, const std::string &cache_dir)
	{
		for (const auto &cpu_cache_dir_entry : directory_iterator(cache_dir))
		{
			...
			boost::split(cache_split, ccd_string, boost::is_any_of("/"));
	...
	cpu_info_t ConstructNewCpuInfo(const std::string &cache_dir, const int level, int core_id, int numa_node)
	{
		...
		return cpu_info_t{{}, level, numa_node,
			...
			for (const auto &cpu_cache_dir_entry : directory_iterator(cache_dir))
			{
				...
				if (level == 3)
				{
					...
					if (it != mask_to_info.end())
						...
						mask_to_info[cpu_map] = ConstructNewCpuInfo(cache_dir, level, std::stoi(core_split.at(0)), numa_node);
				}
			}
		}

		std::vector<cpu_info_t> cache_groups;
