...
PropagationCoefficientsCpu::PropagationCoeffsTopBuriedFull(const SampleConfiguration &sample_config,
...
PropagationCoefficientsCpu::PropagationCoeffsTopBuried(const SampleConfiguration &sample_config,
                                                           ...
                                                           const DetectorConfiguration &detector,
                                                           const BeamConfiguration &beam_config) {
    int qcount = detector_positions.size();
...
#pragma omp parallel for default(none) shared(qcount, detector, detector_positions, beam_config, quad_dist_x, kzi, trans_refs, ns2m1, rki)
    for (int i = 0; i < qcount; ++i) {
        int y = detector_positions.at(i).y;
