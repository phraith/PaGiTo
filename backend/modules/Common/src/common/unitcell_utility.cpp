...
UnitcellV2 GisaxsModeling::CreateFromJson(json unitcell) {
...
UnitcellV2 GisaxsModeling::Convert(FlatUnitcellV2 flat_unitcell) {
    std::vector<std::unique_ptr<Shape>> shapes;
...
FlatUnitcellV2 GisaxsModeling::Convert(UnitcellV2 unitcell) {
    ...
    return {parameters, parameter_indices, upper_bounds, lower_bounds, shape_types, positions, position_indices,
            unitcell.Repetitions(), unitcell.Translation()};
}

std::unique_ptr<Shape> GisaxsModeling::CreateShape(ShapeTypeV2 shape_type, const json &json) {
    std::vector<MyType3> locations;
