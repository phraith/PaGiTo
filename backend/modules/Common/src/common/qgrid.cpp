...
std::string QGrid::InfoStr() const {
    ...
    info += "	-direct beam location in mm (x, y): " + std::to_string(direct_beam_location_.x * pixel_size_) + ", " +
            std::to_string(direct_beam_location_.y * pixel_size_) + "\n";
    ...
    info += "	-n^2 - 1 (2 * delta, 2 * beta): " + std::to_string(refractive_index_.real()) + ", " +
            std::to_string(refractive_index_.imag()) + "\n";
    return info;
}
...
void QGrid::RealSpaceToQ(int x, int y, int i) {
    ...
    /*x0,y0,x1,y1,...*/
    qpoints_xy_.at(i) = {qx, 0};
    qpoints_xy_.at(qcount_ + i) = {qy, 0};
    ...
    /*z00,z01,z02,z03,z10,...*/
    qpoints_z_coeffs_.at(i) = qz0;
    qpoints_z_coeffs_.at(qcount_ + i) = qz1;
    qpoints_z_coeffs_.at(2 * qcount_ + i) = qz2;
    qpoints_z_coeffs_.at(3 * qcount_ + i) = qz3;
    ...
    q_.at(i) = q0;
    q_.at(qcount_ + i) = q1;
    q_.at(2 * qcount_ + i) = q2;
    q_.at(3 * qcount_ + i) = q3;
}
